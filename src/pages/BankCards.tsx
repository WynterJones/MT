...
import AddBankCardForm from '../components/AddBankCardForm';
import EditBankCardForm from '../components/EditBankCardForm';
import { BankCard } from '../types';

const BankCards = () => {
  const { bankCards, deleteBankCard } = useAppData();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-white">Bank Cards</h1>
        <button 
        ...
        <div className="space-y-8">
          {bankCards.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-xl font-bold text-white mb-2">No Bank Cards Yet</h3>
              <p className="text-dark-400 mb-6">Add your first bank account to get started</p>
              <button 
                onClick={() => setShowAddForm(true)}
        ...
        <div className="bg-dark-800 border border-dark-700 rounded-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-dark-700 rounded-lg p-4">
              <h3 className="text-finance-green-400 font-semibold mb-2">Total Accounts</h3>
              <p className="text-3xl font-bold text-white">{bankCards.length}</p>
            </div>
            <div className="bg-dark-700 rounded-lg p-4">
              <h3 className="text-red-400 font-semibold mb-2">Monthly Fees</h3>
              <p className="text-3xl font-bold text-white">
                ${bankCards.reduce((sum, card) => sum + card.monthlyFee, 0).toFixed(2)}
              </p>
            </div>
            <div className="bg-dark-700 rounded-lg p-4">
              <h3 className="text-finance-green-400 font-semibold mb-2">Accounts with Fees</h3>
              <p className="text-3xl font-bold text-white">
                {bankCards.filter(card => card.monthlyFee > 0).length}
              </p>
            </div>
          </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-dark-700">
                      ...
                      <th className="text-left py-2 text-dark-400">Account</th>
                      <th className="text-left py-2 text-dark-400">Type</th>
                      <th className="text-left py-2 text-dark-400">Monthly Fee</th>
                      <th className="text-left py-2 text-dark-400">Min Balance</th>
                  ...
                  <tbody>
                    {bankCards.map((card) => (
                      <tr key={card.id} className="border-b border-dark-700">
                        ...
                        <td className="py-2 text-white">{card.accountName}</td>
                        <td className="py-2 text-white capitalize">{card.accountType}</td>
                        <td className="py-2 text-white">${card.monthlyFee}</td>
                        <td className="py-2 text-white">${card.minimumBalanceForNoFee.toLocaleString()}</td>
                        <td className="py-2">
      ...
      <Modal isOpen={showAddForm} onClose={() => setShowAddForm(false)} title="Add Bank Account">
        <AddBankCardForm onClose={() => setShowAddForm(false)} />
      </Modal>

      <Modal isOpen={showEditForm} onClose={handleCloseEditForm} title="Edit Bank Account">
        {cardToEdit && (
          <EditBankCardForm card={cardToEdit} onClose={handleCloseEditForm} />
        )}
      </Modal>
    </div>
  );
