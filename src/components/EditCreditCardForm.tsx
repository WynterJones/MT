...
const EditCreditCardForm = ({ card, onClose }: EditCreditCardFormProps) => {
  const { updateCreditCard } = useAppData();
  const [formData, setFormData] = useState({
    ...
    associatedBank: card.associatedBank,
    cardColor: card.cardColor,
    status: card.status,
    openedDate: card.openedDate,
    notes: card.notes || '',
    // Rewards
    hasRewards: !!card.rewards,
    rewardsType: card.rewards?.type || 'cashback' as 'cashback' | 'points' | 'miles' | 'other',
    rewardsRate: card.rewards?.rate?.toString() || '',
    rewardsDescription: card.rewards?.description || ''
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateCreditCard(card.id, updatedCard);
    onClose();
  };
